  ...
  switch (block.type) {
    case "h2":
      return (
        <h2
          id={block.id}
          className="mt-12 text-[24px] md:text-[28px]"
    ...
    case "h3":
      return (
        <h3
          id={block.id}
          className={`mt-8 text-[19px] md:text-[22px] ${block.u ? "underline" : ""}`}
          style={{ color: "var(--text-accent)", scrollMarginTop: "7rem" }}
        >
          {block.text}
        </h3>
...
export default function LegalArticle({
  ...
  return (
    <>
      <section data-bg="primary" className="legal-typeset bg-background-primary px-5 pt-32 pb-4 lg:px-12">
      ...
      <section data-bg="primary" className="legal-typeset bg-background-primary px-5 pb-20 md:pb-28 lg:px-12">
        <article className="legal-prose mx-auto max-w-[820px] text-[15px] leading-[1.75] text-text-accent md:text-base">
          {blocks.map((b, i) => (
